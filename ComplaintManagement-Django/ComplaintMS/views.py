...
from .forms import UserRegisterForm, ProfileUpdateForm, UserProfileform, ComplaintForm, UserProfileUpdateform, \
    statusupdate
...
def counter(request):
    ...
    dataset = Complaint.objects.values('Type_of_complaint').annotate(total=Count('status'),
                                                                     ...
                                                                     inprogress=Count('status',
                                                                                      filter=Q(status='2'))).order_by(
        'Type_of_complaint')
